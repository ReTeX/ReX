...
fn write_equations<W: Write>(f: &mut W, old: Equation, new: Equation) {
    ...
    writeln!(f, r#"<pre><code class="language-latex">{}</code></pre>"#, old.tex).unwrap();
...
pub fn write_diff(diff: Vec<(Equation, Equation)>) {
    use std::fs::File;
    use std::io::BufWriter;

    let out = File::create("tests/out/layout.html")
